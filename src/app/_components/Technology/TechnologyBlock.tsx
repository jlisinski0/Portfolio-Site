		...
		<div className='flex flex-col gap-10'>
			{TechnologyItems.map(item => (
				<div className='flex flex-col' key={item.id}>
					<h3 data-aos='fade-up' className='text-base font-medium md:text-xl'>
						{item.heading}
					</h3>
					<div className='flex flex-wrap justify-center lg:justify-start gap-4 pt-8'>
						{TechnologyImg[item.key].map(img => (
							<div key={img.id} data-aos='zoom-in' data-aos-delay='300'>
								<div className='flex items-center justify-center w-25 h-25 rounded-2xl bg-accentTwo hover:scale-115 transition-[scale]'>
